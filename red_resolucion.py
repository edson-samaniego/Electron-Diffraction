...
##############################################################
    #imagen de cv viene en un array
        ...
        imorig= cv2.resize(imorig, None, fx=0.5, fy=0.5)
        h, w, c = imorig.shape
        im_2= imorig.copy()
...
###################### caja bigote #############################
    ...
    #plt.legend()
    #plt.legend(loc='upper right', title='Posicion del pixel')
    ...
    X= ((sum(CX))/(len(CX)))
    Y= ((sum(CY))/(len(CY)))
    pix_cen=(round(Y), round(X))

    cv2.circle(im_2,(round(Y),round(X)),2,rojo,-1)
    im_2[pix_cen[1],pix_cen[0]]= azul
    #plt.title('Final: centro promedio procesado')
    #plt.imshow(im_2)    ###################
    #plt.show()          ####################
    #print(pix_cen)
    
    im_3= cv2.resize(im_2, None, fx=2, fy=2)
    #plt.title('Final: imagen con centro restaurada')
    #plt.imshow(im_3)    ###################
    #plt.show()          ####################
    pix_cen=((pix_cen[1]*2),(pix_cen[0]*2))
